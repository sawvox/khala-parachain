...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub(crate) fn try_service_message(
            ...
            log::debug!("phala-debug: try_service_message: {:?}", data);
	...
	impl<T: Config> DmpMessageHandler for Pallet<T> {
		fn handle_dmp_messages(
            ...
            log::debug!("phala-debug: handle_dmp_messages: {:?}", limit);
			...
			for (i, (sent_at, data)) in iter.enumerate() {
                log::debug!("phala-debug: handle_dmp_messages::iter: {:?}", (sent_at, data));
